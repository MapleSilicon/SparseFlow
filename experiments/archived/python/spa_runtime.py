#!/usr/bin/env python3
"""
SPA v0.7 Runtime - Masked MatMul with Real Speedup
"""
import numpy as np
import json
import time
...
def load_spa_masks(json_path: str) -> Dict:
    """Load SPA sparsity masks from JSON."""
...
def spa_masked_matmul(A: np.ndarray, B: np.ndarray, 
    ...
    Skip:
    - rows where rowmask[i] = False (row i of A is all zeros)
    - cols where colmask[j] = False (col j of B is all zeros)
    """
    M, K1 = A.shape
    K2, N = B.shape
...
def main():
    print("=" * 60)
    print("SPA v0.7 Runtime - Masked MatMul Benchmark")
    print("=" * 60)
    print()
    
    # Load SPA masks
    masks = load_spa_masks('spa_sparsity.json')
    if not masks:
        print("❌ No matmul found in spa_sparsity.json")
        return
    
    print(f"✅ Loaded SPA masks:")
    print(f"   Row mask: {masks['rowmask']}")
    print(f"   Col mask: {masks['colmask']}")
    print(f"   Row sparsity: {masks['row_sparsity']:.0f}%")
    print(f"   Col sparsity: {masks['col_sparsity']:.0f}%")
    print()
    
    # Create test matrices matching the mask size
    M = len(masks['rowmask'])
    N = len(masks['colmask'])
    K = M  # Square-ish for simplicity
    ...
    # Apply the masks to input matrices (zero out the sparse rows/cols)
    for i, active in enumerate(masks['rowmask']):
        if not active:
            A[i, :] = 0
    
    for j, active in enumerate(masks['colmask']):
        if not active:
            B[:, j] = 0
    ...
    dense_time, C_dense = benchmark_matmul(dense_matmul, A, B)
    print(f"⚙️  Dense matmul:  {dense_time*1000:.3f} ms")
    
    # Benchmark SPA-guided sparse matmul
    sparse_time, C_sparse = benchmark_matmul(
        spa_masked_matmul, A, B, 
        masks['rowmask'], masks['colmask']
    )
    print(f"⚡ Sparse matmul: {sparse_time*1000:.3f} ms")
    print()
    
    # Verify correctness
    max_diff = np.max(np.abs(C_dense - C_sparse))
    print(f"✅ Correctness check: max difference = {max_diff:.2e}")
    print()
    ...
    active_rows = sum(masks['rowmask'])
    active_cols = sum(masks['colmask'])
    total_elements = M * N
    computed_elements = active_rows * active_cols
    theoretical_speedup = total_elements / computed_elements if computed_elements > 0 else 0
    ...
    print()
    print(f"Active computation: {computed_elements}/{total_elements} elements")
    print(f"                    ({computed_elements/total_elements*100:.1f}% of dense)")
    print("=" * 60)
