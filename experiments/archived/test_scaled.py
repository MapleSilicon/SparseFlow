...
print("="*60)
print("SCALED KERNEL TEST (128×128 tile, 8 warps)")
print("="*60)
...
if err < 0.01:
    ...
    def bench(M):
        ...
        for _ in range(10): lib.launch_gemm_ptx_scaled(A.data_ptr(), B.data_ptr(), C.data_ptr(), M, M, M)
        torch.cuda.synchronize()
        
        s = time.perf_counter()
        for _ in range(50): lib.launch_gemm_ptx_scaled(A.data_ptr(), B.data_ptr(), C.data_ptr(), M, M, M)
        torch.cuda.synchronize()
        
        return (2*M**3*50)/(time.perf_counter()-s)/1e12
    
    print("Performance (NO cp.async yet):")
    for size in [2048, 4096, 8192]:
        print(f"  {size}×{size}: {bench(size):.2f} TFLOPS")
else:
    print(f"❌ FAILED")
