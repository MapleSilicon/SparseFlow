...
class SparseFlowLinear(nn.Module):
    ...
    def __init__(
        ...
        # Store ORIGINAL weights for fallback (critical for performance!)
        self.register_buffer("W_original", weight_fp16.contiguous(), persistent=True)
        
        # Store pruned weights for sparse path
        W_pruned = prune_24_dense_weight(weight_fp16.contiguous())
        self.register_buffer("W_dense_pruned", W_pruned, persistent=True)
        # CRITICAL: Must be a buffer to survive torch.compile
        self.register_buffer('W_sparse', torch.sparse.to_sparse_semi_structured(self.W_dense_pruned), persistent=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Sparse path: use tensor cores
        if use_sparse:
            self._sparse_count += 1
            # Use _sparse_semi_structured_mm - try without contiguous
            Ws = self.W_sparse
            ...
            # Fallback: use ORIGINAL dense weights (not pruned!)
            self._dense_count += 1
            y2d = F.linear(x2d, self.W_original, self.bias)
