...
__global__ void gemm_sparse_minimal(
    const half* __restrict__ A_values,
    const uint32_t* __restrict__ A_meta,
    const half* __restrict__ B,
    float* __restrict__ C,
    int M, int N, int K
) {
    __shared__ __align__(16) half smem_A[BM][BK/2];
    __shared__ __align__(16) half smem_B[BK][BN];
    __shared__ __align__(16) uint32_t smem_meta[BM][2]; // Simplified metadata storage
    ...
    // Load compressed A values (16×16)
    ...
    // Load metadata (16 rows × 2 uint32)
    for (int i = lane; i < BM * 2; i += 32) {
        int r = i / 2;
        int c = i % 2;
        int gr = block_row + r;
        smem_meta[r][c] = (gr < M) ? A_meta[gr * 2 + c] : 0;
    }
    ...
    // Load metadata - FIX: Add .x2 shape modifier
    asm volatile(
    ...
    // SPARSE MMA - FIX: Use correct argument format
    // The .sp variant needs metadata in specific format
    asm volatile(
